...
```bash
go get github.com/daheige/bridge-svc/bridge
```

```go
import "github.com/daheige/bridge-svc/bridge"
```

### 运行示例

项目已提供示例：`bridge/example/main.go`。
...
import (
    ...
    "github.com/daheige/bridge-svc/bridge"
)
...
client, err := bridge.NewClient(cfg,
    ...
    bridge.WithDialOptions(grpc.WithBlock()),
    bridge.WithServiceDialOpts("greeter-svc", grpc.WithIdleTimeout(10*time.Minute)),
)
```
